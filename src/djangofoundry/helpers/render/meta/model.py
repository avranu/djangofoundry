...
class ConstraintInfo(DbInfo):
    ...
    def __init__(
        ...
        r_constraint_name: str | None = None,
        r_owner: Any | None = None,
...
class IndexInfo(DbInfo):
    ...
    def __init__(self, name: str, columns: list[IndexColumnInfo] | None = None, uniqueness: str = ""):
        if not name:
...
class ForeignKeyInfo:
    ...
    def __hash__(self) -> int:
        return hash(
            (self.column, self.constraint_type, self.referenced_table, self.referenced_column, self.related_name)
        )
