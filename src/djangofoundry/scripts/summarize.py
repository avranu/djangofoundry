...
class PythonClassParser:
    ...
    @classmethod
    def parse_file(cls, file: str) -> ast.Module | None:
        """
    ...
    @classmethod
    def group_classes_by_module(
        cls, classes: dict[str, list[dict[str, Any]]], module_name: str
    ) -> list[dict[str, Any]]:
        """
        ...
        with open(self.output_file, "a") as f:
            for module_name, classes in grouped_classes.items():
                ...
                for class_data in classes:
                    try:
                        base_classes_str = (
                            "(" + ", ".join(class_data["base_classes"]) + ")" if class_data["base_classes"] else ""
                        )
                        ...
                        logger.debug(
                            f"Wrote {class_data['name']} and {len(class_data['methods'])} methods to {self.output_file}"
                        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--recursive",
        action="store_true",
        help="Enable or disable recursive search in subdirectories",
        default=True,
    )
