...
class QuerySet(auto_prefetch.QuerySet):
    ...
    def min(self, property_name: str) -> float | None:
        """
    ...
    def max(self, property_name: str) -> float | None:
        """
    ...
    def total(self, property_name: str) -> float | int | Decimal:
        """
    ...
    def request(self, request_str: str) -> Self | RawQuerySet:
        """
    ...
    def apply_filter(self, filter_name: str) -> Self | RawQuerySet:
        """
    ...
    def filter_by_related(self, foreign_key: str, filter_expression: Q, alias_name: str | None = None) -> Self:
        """
    ...
    def summarize_distribution(self, field_name: str, bins: int = 10) -> dict[int, int]:
        ...
        distribution = (
            ...
            .annotate(
                bin=ExpressionWrapper(F(field_name) - min_value, output_field=IntegerField())
                // ((max_value - min_value) // bins)
            )
            .values("bin")
    ...
    def summarize_x_by_y_distribution(
        self, x_field_name: str, y_field_name: str, bins: int = 10
        ...
        distribution = (
            ...
            .annotate(
                bin=ExpressionWrapper(F(y_field_name) - min_value, output_field=IntegerField())
                // ((max_value - min_value) // bins)
            )
    ...
    def median(self, field_name: str) -> float | None:
        ...
        if row_count % 2 == 1:
            ...
            median_value = (
                ...
                .aggregate(median_value=Avg(ExpressionWrapper(F(field_name), output_field=FloatField())))[
                    "median_value"
                ]
            )
        return median_value
    ...
    def filter_median(self, field_name: str, deviation: int = 0) -> Self:
        ...
        if standard_deviation is None:
            logger.error(
                f"Unable to calculate standard deviation for field {field_name} in queryset {self.model.__name__}"
            )
            return self.none()

        return self.filter(
            **{
                f"{field_name}__gte": median - (standard_deviation * deviation),
                f"{field_name}__lte": median + (standard_deviation * deviation),
            }
        )
    ...
    def filter_percentile(self, field_name: str, percentile: float, deviation: int = 0) -> Self:
        ...
        return self.filter(
            **{
                f"{field_name}__gte": result - (standard_deviation * deviation),
                f"{field_name}__lte": result + (standard_deviation * deviation),
            }
        )
    ...
    def mode(self, field_name: str) -> float:
        ...
        mode_value = max(value_counts, key=operator.itemgetter("count"))[field_name]
        return mode_value
    ...
    def filter_mode(self, field_name: str, deviation: int = 0) -> Self:
        ...
        return self.filter(
            **{
                f"{field_name}__gte": mode - (standard_deviation * deviation),
                f"{field_name}__lte": mode + (standard_deviation * deviation),
            }
        )
    ...
    def filter_mean(self, field_name: str, deviation: int = 0) -> Self:
        ...
        return self.filter(
            **{
                f"{field_name}__gte": mean - (standard_deviation * deviation),
                f"{field_name}__lte": mean + (standard_deviation * deviation),
            }
        )
    ...
    def covariance(self, field_name_x: str, field_name_y: str) -> float:
        ...
        # Get the covariance
        covariance = self.aggregate(covariance=Avg((F(field_name_x) - mean_x) * (F(field_name_y) - mean_y)))[
            "covariance"
        ]
        return covariance
    ...
    def find_correlated_fields(self, field_name: str, threshold: float = 0.5) -> list[str]:
        ...
        correlations = {
            correlated_field_name: self.correlation(field_name, correlated_field_name)
            for correlated_field_name in self.model._meta.get_fields()
            if (field_name != correlated_field_name and self.field_is_numeric(correlated_field_name))
        }
        correlations = {
            correlated_field_name: correlation
            for correlated_field_name, correlation in correlations.items()
    ...
    def search(self, search_term: str, fields: list[str] | None = None) -> Self:
        """
    ...
    def filter_anomalies_in(
        self, field_name: str, method: str | None = None, z_threshold: float = 2, iqr_multiplier: float = 1.5
    ) -> Self:
        """
    ...
    def filter_anomalies(self, method: str | None = None, z_threshold: float = 2, iqr_multiplier: float = 1.5) -> Self:
        """
        ...
        for _i, field in enumerate(numeric_fields):
            filter_q = Q(
                **{
                    f"{field}__in": self.filter_anomalies_in(field, method, z_threshold, iqr_multiplier).values_list(
                        field, flat=True
                    )
                }
            )
